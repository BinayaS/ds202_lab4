...
defense <- read_excel('D:\\Personal Files\\School Work 2020 Fall\\Ds 202\\Lab 4\\ds202_lab4\\cyclonesFootball2019.xlsx', sheet='Defensive')
offensive <- read_excel('D:\\Personal Files\\School Work 2020 Fall\\Ds 202\\Lab 4\\ds202_lab4\\cyclonesFootball2019.xlsx', sheet='Offensive')
biography <- read_excel('D:\\Personal Files\\School Work 2020 Fall\\Ds 202\\Lab 4\\ds202_lab4\\cyclonesFootball2019.xlsx', sheet='Biography')
defense2018 <- read_excel('D:\\Personal Files\\School Work 2020 Fall\\Ds 202\\Lab 4\\ds202_lab4\\cyclonesFootball2019.xlsx', sheet='Defensive')
offensive2018 <- read_excel('D:\\Personal Files\\School Work 2020 Fall\\Ds 202\\Lab 4\\ds202_lab4\\cyclonesFootball2019.xlsx', sheet='Offensive')
```
...
* The most rare defensive skills are Turnover_INT, Turnover_FR, and Turnover_FF due to them having the lowest occurrences for any player.
...
ggplot(defCount, aes(x = stat, y = count)) + geom_bar(position = 'dodge', stat='identity') + geom_text(aes(label=count), position=position_dodge(width=0.9), vjust=-0.25) + theme(axis.text.x=element_text(angle = -90, vjust = 0.5))
```
...
* The defense against Notre Dame was better because there is a higher count of solo tackles more often
...
ggplot(numPlayersFromState, aes(x = Var1, y = Freq)) + geom_bar(position = 'dodge', stat='identity') + geom_text(aes(label=Freq), position=position_dodge(width=0.9), vjust=-0.25) + theme(axis.text.x=element_text(angle = -90, vjust = 0.5))
```
...
* The players with big improvements for the offensive team are 
...
* The players with big improvements for the defensive team are
